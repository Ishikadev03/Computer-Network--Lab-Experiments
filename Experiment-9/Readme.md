...
Topology Design: Designed a network topology with PCs and a Server connected via a Switch.
IP Addressing: Assigned static IP Addresses to the PCs and the Server.
TCP Simulation: Configured an HTTP service on the Server. Accessed the server via the PC's web browser to generate TCP traffic.
UDP Simulation: Configured a DNS service on the Server. Ran a domain query from the PC to generate UDP traffic.
Traffic Filtering: Switched to Simulation Mode and filtered the Event List to show only TCP and UDP packets.
Header Inspection: Clicked on the respective Protocol Data Units (PDUs) to inspect the transport layer headers, port numbers, and connection flags.
Configuration Commands
N/A (Configurations handled via the Server's Service tab and PC desktop applications).

Observations / Results
TCP Handshake Headers
